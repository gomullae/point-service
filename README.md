...
단순 잔액 테이블이 아니라 **적립 단위별 원장 추적**을 중심으로 설계했습니다.
...
적립 요청의 동일성 판단에는 사용자, 금액, 적립 타입, 만료일을 포함합니다.  
...
사용 취소는 사용 당시 차감된 순서(`use_sequence` 오름차순)대로 복원합니다.  
역순이 아닌 사용 순서로 복원하는 이유는, 취소 금액을 처음 차감된 적립분부터 순서대로 배분해야 어떤 적립분이 얼마 복원되었는지 추적이 명확하기 때문입니다.  
예를 들어 A(1000pt) → B(200pt) 순서로 차감했다면, 취소 금액도 A부터 먼저 채웁니다.
